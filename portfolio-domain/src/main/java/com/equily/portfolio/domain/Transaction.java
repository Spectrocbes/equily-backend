...
public record Transaction(
    ...
    private static final Set<TransactionType> ASSET_TYPES = EnumSet.of(
            TransactionType.BUY, TransactionType.SELL
    );
    private static final Set<TransactionType> CASH_ONLY_TYPES = EnumSet.of(
            TransactionType.DEPOSIT, TransactionType.WITHDRAWAL
    );

    public Transaction {
        // validation is enforced in the static factory of(...); direct construction is intentionally left open
    ...
    public static Transaction of(
        ...
        if (ASSET_TYPES.contains(type)) {
            if (ticker == null) throw new InvalidTransactionException("ticker must not be null for " + type);
            if (quantity == null) throw new InvalidTransactionException("quantity must not be null for " + type);
            if (quantity.compareTo(BigDecimal.ZERO) <= 0) {
                throw new InvalidTransactionException("quantity must be positive for " + type);
            }
            if (pricePerUnit == null) throw new InvalidTransactionException("pricePerUnit must not be null for " + type);
        }
